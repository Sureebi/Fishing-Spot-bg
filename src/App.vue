...
import { fishingShops, fishingSpots } from './data/fishingSpots';
...
const filteredSpots = computed(() =>
  fishingSpots.filter((spot) => {
    const matchesType = filters.value.type === 'all' || spot.type === filters.value.type;
    const matchesAccess = filters.value.access === 'all' || spot.access === filters.value.access;
  ...
  <main class="app-layout">
    <section class="sidebar" aria-label="Контролен панел">
      ...
      <div class="status-strip">
        <span>{{ filteredSpots.length }} места</span>
